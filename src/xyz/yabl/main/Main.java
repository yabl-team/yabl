			...
			if (session.getUri().startsWith("/api/whoami")) {
				...
				if (session.getUri().startsWith("/api/bot/")) {
								...
								if (session.getUri().split("/")[4].equals("add")) {
										...
										try (CloseableHttpClient httpclient = HttpClients.createDefault()) {
											...
											try {
												...
												if (response.has("username") && response.get("id").getAsString().equals(json.get("id").getAsString()) && response.has("bot")) {
													...
													bots.forEach(a -> {
														botsUpdated.add(new BsonString(a));
													});
									...
									try {
										if (json.get("prefix") != null) {
											...
											document.put("prefix", bot.get("prefix"));
										}
										if (json.get("help") != null) {
											document.put("help", json.get("help").getAsString());
										} else {
											document.put("help", bot.get("help"));
										}
										if (json.get("body") != null) {
											document.put("body", json.get("body").getAsString());
										} else {
											document.put("body", bot.get("body"));
										}
										if (json.get("desc") != null) {
											document.put("desc", json.get("desc").getAsString());
										} else {
											document.put("desc", bot.get("desc"));
										}
										if (json.get("website") != null) {
											document.put("website", json.get("website").getAsString());
										} else {
											document.put("website", bot.get("website"));
										}
										if (json.get("support") != null) {
											document.put("support", json.get("support").getAsString());
										} else {
											document.put("support", bot.get("support"));
										}
										if (json.get("git") != null) {
											document.put("git", json.get("git").getAsString());
										} else {
											document.put("git", bot.get("git"));
										}
										if (json.get("library") != null) {
											document.put("library", json.get("library").getAsString());
										} else {
											document.put("library", bot.get("library"));
										}
										if (json.get("modnote") != null) {
					...
					if (session.getUri().startsWith("/api/bots/user/")) {
							...
							if (session.getUri().startsWith("/api/bots/user/@me")) {
								if (authorization != null && (this.loggedUsers.containsKey(authorization) || apiToken)) {
									...
									if (user.get("bots") instanceof ArrayList<? > ) {
										((ArrayList<? > ) user.get("bots")).forEach((a -> {
											botIds.add(a.toString());
										}));
										b.put("$in", botIds);
										d.put("id", b);
									} else {
								...
								if (user.get("bots") instanceof ArrayList<? > ) {
									((ArrayList<? > ) user.get("bots")).forEach((a -> {
										botIds.add(new BsonString(a.toString()));
									}));
									b.put("$in", botIds);
									d.put("id", b);
								} else {
						...
						List<String> bots = new ArrayList<>();
						Document[] aggregateArray = {
							Document.parse("{ $match : {\"verified\":true,\"guildCount\":{\"$exists\":true},\"support\":{\"$type\":2}}}"),
